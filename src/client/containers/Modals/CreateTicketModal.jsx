  ...
  onTicketTypeSelectChange (e) {
    ...
    axios
      ...
      .catch(error => {
        this.priorityLoader.classList.add('hide')
        Log.error(error)
        helpers.UI.showSnackbar(`${this.props.t('Error')}: ${error.response.data.error}`)
      })
  }
        ...
        <form className={'uk-form-stacked'} onSubmit={e => this.onFormSubmit(e)}>
          <div className='uk-margin-medium-bottom'>
            ...
            <input
              ...
              data-validation-error-msg={`${this.props.t('Please enter a valid Subject. Subject must contain at least')}
                 ${viewdata
                .get('ticketSettings')
                .get('minSubject')} ${this.props.t('characters.')}`}
            />
          </div>
          <div className='uk-margin-medium-bottom'>
            <Grid>
              {allowAgentUserTickets && (
                <GridItem width={'1-3'}>
                  <label className={'uk-form-label'}>{this.props.t('Owner')}</label>
                  <SingleSelect
                    showTextbox={true}
          ...
          <div className='uk-margin-medium-bottom'>
            <label className={'uk-form-label'}>{this.props.t('Priority')}</label>
            <div
          ...
          <div className='uk-margin-medium-bottom'>
            ...
            <span style={{ marginTop: '6px', display: 'inline-block', fontSize: '11px' }} className={'uk-text-muted'}>
              {this.props.t('Please try to be as specific as possible. Please include any details you think may be' +
                ' relevant, such as troubleshooting steps you\'ve taken.')}
              {/* eslint-disable-next-line react/no-unescaped-entities */}
            </span>
          </div>
          <div className='uk-modal-footer uk-text-right'>
            <Button text={this.props.t('Cancel')} flat={true} waves={true} extraClass={'uk-modal-close'} />
            <Button text={this.props.t('Create')} style={'primary'} flat={true} type={'submit'} />
          </div>
        </form>
      </BaseModal>
    )
